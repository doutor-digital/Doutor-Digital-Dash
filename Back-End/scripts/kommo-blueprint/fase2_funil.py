...
RENAMES = [
    ...
    {"legacy": "07_NAO_FECHOU_TRATAMENTO",           "name": "COMPARECEU",             "color": "#deff81", "tmp": 920, "sort": 40},
    {"legacy": "09_AGUARDANDO_RETORNO_POS_CONSULTA", "name": "RETORNO PÓS-TRATAMENTO", "color": "#ccc8f9", "tmp": 930, "sort": 60},
]

"""  Etapas canônicas sem âncora legada — criadas do zero no cleanup.  """
NOVAS_ETAPAS = [{"name": "EM NEGOCIAÇÃO", "color": "#ff8f92", "sort": 50}]
...
"""  De-para aprovado pelo dono. `to` = (funil, etapa) canônica; `loss` = motivo de
     perda pelo NOME (resolvido no lock); `set` = campos escritos junto com a mudança.  """
DEPARA = {
    ...
    "06_FALTOU_CONSULTA":                 {"to": (PIPELINE_COMERCIAL, "AGENDADO"),
                                           "set": {"SPINE_SCHEDULE_STATUS": "Não compareceu"}},
    "08_EM_TRATAMENTO":                   {"to": (PIPELINE_TRATAMENTO, "EM TRATAMENTO")},
    "10_CANCELAMENTO_TRATAMENTO":         {"to": (PIPELINE_TRATAMENTO, 143)},
    "11_ALTA_SATISFEITO":                 {"to": (PIPELINE_TRATAMENTO, 142)},
    "12_ALTA_INSATISFEITO":               {"to": (PIPELINE_TRATAMENTO, 142)},
    "13_NAO_PERTURBAR":                   {"to": (PIPELINE_COMERCIAL, 143), "loss": "Não perturbar"},
    "14_ENCAMINHADO_MAGALHAES_ADVOCACIA": {"to": (PIPELINE_COMERCIAL, 143), "loss": "Caso enviado para a franquia"},
    "15_NAO_DEU_CONTINUIDADE":            {"to": (PIPELINE_COMERCIAL, 143), "loss": "Não deu continuidade ao atendimento"},
    "16_LEAD_MORTO":                      {"to": (PIPELINE_COMERCIAL, 143), "loss": "Lead morto"},
}
...
def step_cleanup(cli, args):
    ...
    canonicas = {r["name"] for r in RENAMES} | {n["name"] for n in NOVAS_ETAPAS}
    ...
    for nova in NOVAS_ETAPAS:
        if status_by_name(com, nova["name"]):
            print(f"  = etapa {nova['name']!r} já existe")
            continue
        print(f"  + etapa {nova['name']!r}")
        if args.apply:
            cli.post(f"leads/pipelines/{com['id']}/statuses", [nova])
...
def main() -> int:
    ...
    ap.add_argument(
        "--config",
        type=Path,
        help="JSON {renames, novas_etapas, depara} da unidade. Sem isto usa o mapa "
        "embutido (Canaã). O de-para de fechamento/no-show muda por unidade — "
        "sempre confirmar com o dono antes (gotcha #10).",
    )
    args = ap.parse_args()
    ...
    if args.config:
        ...
        if "renames" in cfg:
            globals()["RENAMES"] = cfg["renames"]
        if "novas_etapas" in cfg:
            globals()["NOVAS_ETAPAS"] = cfg["novas_etapas"]
        if "depara" in cfg:
            globals()["DEPARA"] = {k: {**v, "to": tuple(v["to"])} for k, v in cfg["depara"].items()}
